...
```{r}
coeftest(arima.best.AIC_4)

coeftest(arima.best.AIC_8)
```
...
How many lags should we include in Ljung-Box test? We aim using as many lags as we need to capture 'any meaningful and troublesome correlations'.
...
```
```{r}
AIC(arima.best.BIC_4, arima314_4)
```
```{r}
BIC(arima.best.BIC_8, arima314_4)
```
```{r}
AIC(arima.best.AIC_4, arima213_4)
```
```{r}
BIC(arima.best.AIC_8, arima415_8)
```
Based on AIC and BIC criterions best models are as follows:

For x4 ARIMA(3,1,0)

For x8 ARIMA(2,1,0)

These models have lower AIC and BIC compare to manual selected models.
...
time_index <- seq(start_date, length.out = nrow(forecasts_data), by = "day")

forecasts_xts4 <- xts(forecasts_data4, order.by = time_index)

forecasts_xts8 <- xts(forecasts_data8, order.by = time_index)
